...
PKGS="asciinema bash-completion blender cmake distrobox elogind ffmpeg firefox fuzzel fzf git imv intel-media-driver jq mako mesa mpv nemo ninja niri noto-fonts-cjk noto-fonts-cjk-variable noto-fonts-emoji noto-fonts-ttf noto-fonts-ttf-variable nushell openntpd python3 python3-devel rio ripgrep rustup starship swww uv vulkan-loader wireplumber xdg-desktop-portal-gnome xdg-user-dirs xtools xwayland-satellite yt-dlp zoxide"
...
CRYPT_UUID=$(blkid --match-tag UUID --output value /dev/disk/by-partlabel/cryptroot)
...
if [[ ! -z "$CMDLINE_DEFAULT" ]]; then
  CMDLINE_DEFAULT="$CMDLINE_DEFAULT rd.auto=1 rd.luks.name=$CRYPT_UUID=cryptroot rd.luks.allow-discards=$CRYPT_UUID"
else
  CMDLINE_DEFAULT="rd.auto=1 rd.luks.name=$CRYPT_UUID=cryptroot rd.luks.allow-discards=$CRYPT_UUID"
fi

echo "GRUB_ENABLE_CRYPTODISK=y" >> /etc/default/grub
sed -i "s/GRUB_CMDLINE_LINUX_DEFAULT.*/GRUB_CMDLINE_LINUX_DEFAULT=\"$CMDLINE_DEFAULT\"/" /etc/default/grub

dd bs=512 count=4 if=/dev/urandom of=/boot/cryptroot.key
echo $LUKSPASS | cryptsetup luksAddKey /dev/disk/by-partlabel/cryptroot /boot/cryptroot.key &> /dev/null
echo "cryptroot UUID=$CRYPT_UUID /boot/cryptroot.key luks" > /etc/crypttab
chmod 000 /boot/cryptroot.key
...
install_items+=" /boot/cryptroot.key /etc/crypttab "
EOF
...
ln -srf /etc/sv/{dbus,elogind,grub-btrfs,NetworkManager,openntpd} /var/service/
...
useradd --create-home --groups wheel,users,input,audio,video,network --shell /bin/bash $USERNAME

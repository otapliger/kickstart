...
PKGS="alacritty asciinema bash-completion bat bottom cmake elogind emacs-pgtk ffmpeg fish-shell fzf git httptap hunspell hunspell-en imv intel-media-driver jq librewolf mako meld mesa mesa-vulkan-intel nemo ninja niri noto-fonts-cjk noto-fonts-cjk-variable noto-fonts-emoji noto-fonts-ttf noto-fonts-ttf-variable openntpd papirus-folders papirus-icon-theme pipewire podman podman-tui python3 python3-devel rio ripgrep ruff rustup sv-helper starship swww tmux ufw vim vulkan-loader wireplumber xdg-desktop-portal-gnome xdg-user-dirs xwayland-satellite yq-go yt-dlp zoxide"
...
yes | xi && yes | xbps-install -USy --repository "$REPO" $PKGS

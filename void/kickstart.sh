...
if [ "$(id -u)" -ne 0 ]; then
  echo "Error: root privileges is required. Please re-run the script as root." >&2
  exit 1
fi
...
parted -s "$DISK" \
  ...
  mkpart ENCRYPTED 513MiB 100% \
  set 1 esp on

ESP="/dev/disk/by-partlabel/ESP"
CRYPTROOT="/dev/disk/by-partlabel/ENCRYPTED"
...
echo -n "$LUKSPASS" | cryptsetup luksOpen "$CRYPTROOT" "$HOSTNAME" -d -
VOID="/dev/mapper/$HOSTNAME"
